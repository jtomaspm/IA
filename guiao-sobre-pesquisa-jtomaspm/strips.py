...
class STRIPS(SearchDomain):
    ...
    def result(self, state, action):
        #garantir que action é aplicavel ao estado
        #ou seja o estado cumpre todas as pre condições
        if not all(pc in state for pc in action.pc):
            return None

        #remover efeitos negativos ao estado
        newstate = [c for c in state if c not in action.neg]

        #adicinar efeitos positivos ao estado
        newstate.extend(action.pos)

        #garantir que não ha estados duplicados
        return set(newstate)
